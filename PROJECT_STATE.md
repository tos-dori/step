...
- 목록 카드를 보는 동작과 현재 Step으로 올리는 동작을 분리한다. 현재 Step 전환은 별도의 재생 아이콘 버튼처럼 명확한 실행 기호로 표현하고, 텍스트 라벨은 가능한 한 줄인다.
...
- 새 할 일 목록·상세·현재 Step 사이의 정확한 화면 전환과 PC 2단 레이아웃은 아직 patch ticket으로 확정하지 않았다.
...
6. 데이터 보존 문제가 안정된 뒤 tester에서 기본 할 일 목록 → 재생 아이콘으로 현재 Step 전환 → 목록에서 직접 완료 → 모바일 단일 열·PC 확장 레이아웃 순서로 유저 흐름과 patch ticket을 확정한다.
