(function(){
  ...
  function checkpointRaw(raw,reason,force){
    ...
    if(latest&&latest.hash===hash)return true;
  ...
  function write(key,nextRaw,reason){
    ...
    var normalized=JSON.stringify(next),current=safeGet(key),why=String(reason||"local-change"),forced=forceReason(why);
    if(current&&current!==normalized){
      var checkpointed=checkpointRaw(current,why,forced);
      if(forced&&!checkpointed){blocked=true;issue="복구본을 남기지 못해 이 변경을 중단했어요.";return false}
    }
